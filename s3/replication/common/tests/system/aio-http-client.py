...
async def test_case_1(short_label, total_count):
    ...
    for i in range(total_count):
        ...
        resp = await async_put_api(client_session, uri)
...
async def main():
    ...
    # Sequential requests.
    await test_case_1("seq-1", total_count=1000)

    # Parallel requests - all
    await test_case_2("parallel-1", total_count=1000, max_conn=100)
    await test_case_2("parallel-2", total_count=1000, max_conn=10)
    await test_case_2("parallel-3", total_count=1000, max_conn=1)

    # Parallel requests - grouped
    await test_case_3("grouped-1", total_count=1000, max_conn=100, group_size=10)
    await test_case_3("grouped-2", total_count=1000, max_conn=100, group_size=5)
    await test_case_3("grouped-3", total_count=1000, max_conn=100, group_size=1)
