...
async def main():
    ...
    aiohttp.DefaultResolver = aiohttp.resolver.AsyncResolver
    connector = aiohttp.TCPConnector(limit=config.max_s3_connections,
        ttl_dns_cache=300)
    ...
    for i in range(total_count):
        ...
        # resp = await async_put_object(client_session, uri, data, object_size)
        # print("triggered async_put_object for {}".format(object_name))
        # put_responses.append(resp)
    # await asyncio.sleep(0)
    # Wait for uploads to complete.
    # print("Waiting for completion")
    # await asyncio.sleep(100)
    # await put_task_list[0]
    put_responses = await asyncio.gather(*put_task_list)
    # for i in range(total_count):
    #     await put_task_list[i]
